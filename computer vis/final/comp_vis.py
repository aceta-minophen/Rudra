...
def encode_faces():
    ...
    for name in os.listdir(KNOWN_FACES_DIR):
        ...
        image = face_recognition.load_image_file(
            os.path.join(f'{KNOWN_FACES_DIR}', f'{name}'))
...
def process_video():
    ...
    while video_capture.isOpened():
        ...
        for (top, right, bottom, left), name in zip(face_locations, face_names):
            ...
            if name != 'Unknown':
                ref.update(
                    {'Face Recognition/User': {'Name': name, 'Recognized': True}})
                #captured_emotions = emo_detector.detect_emotions(frame)
                dominant_emotion, emotion_score = emo_detector.top_emotion(
                    frame)
                if dominant_emotion == 'happy':
                    ref.update({'Expression Detection': {
                               'Happy': True, 'Upset': False}})
                elif dominant_emotion == 'sad' or dominant_emotion == 'angry':
                    ref.update({'Expression Detection': {
                               'Happy': False, 'Upset': True}})
                else:
                    ref.update({'Expression Detection': {
                               'Happy': False, 'Upset': False}})
