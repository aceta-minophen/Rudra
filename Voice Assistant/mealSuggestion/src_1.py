...
def insert_row_in_orders(title="", orders=orders):
    ...
    upper_row = pd.DataFrame({'order_id': [0], 'food_id': [idx[0]], 'canteen_id': [
                             1], 'user_id': [1]}, index=[0])
...
def avg_rating(user_rating, m=""):
    ...
    if df1.loc[idx[0], 'user_rating'] == 0:
        ...
        df1.loc[idx, 'avg_rating'] = ((df1.loc[idx, 'num_rating'].multiply(
...
def insert_row(user_rating, category="", title="", df1=df1):
    ...
    top_row = pd.DataFrame({'food_id': [0], 'title': [title], 'canteen_id': [1], 'num_orders': [1], 'avg_rating': [
                           user_rating], 'num_rating': [1], 'category': [category], 'tags': ['healthy'], 'user_rating': [user_rating]}, index=[0])
...
df1 = check_foodname(7.5, "main course", "roti")
...
orders = insert_row_in_orders("banana")
...
df2 = df1.loc[get_recommendations(title="pasta")]
